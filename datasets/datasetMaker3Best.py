...
def build_datasets_parallel(df):
    ...
    samples_per_region_year = {h: defaultdict(lambda: defaultdict(int)) for h in HORIZONS}  # h -> region -> year -> count
    samples_per_region_month = {h: defaultdict(lambda: defaultdict(int)) for h in HORIZONS}  # h -> region -> (year, month) -> count
    ...
    print("Building datasets for horizons in parallel in batches...")

    while remaining_dates:
        batch_dates = remaining_dates[:BATCH_SIZE]
        date_data_list = [prepare_date_data(date, date_files) for date in batch_dates]

        with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
            for date, partial_results in tqdm(executor.map(process_one_date, date_data_list), total=len(date_data_list)):
                if date in flushed_dates:
                    continue  # Already flushed

                # Early stop if quotas met
                if all(
                    ...
                    remaining_dates = []  # break outer while
                    break

                # Add samples, respecting quotas
                for h in HORIZONS:
                    for sample in partial_results[h]:
                        ...
                        if samples_per_region_horizon[h][region] >= MAX_SAMPLES_PER_REGION_PER_HORIZON:
                            continue
                        if samples_per_region_year[h][region][year] >= MAX_SAMPLES_PER_REGION_PER_YEAR:
                            continue
                        if samples_per_region_month[h][region][ym_key] >= MAX_SAMPLES_PER_REGION_PER_MONTH:
                            continue

                        datasets[h].append(sample)
                        samples_per_region_horizon[h][region] += 1
                        samples_per_region_year[h][region][year] += 1
